...
xticks([0 : 0.1 : 1.2]);
set(gca,'Xticklabel',[]) 
ylabel("Free surface, $h(x, t)$", "Fontsize", fontsize, "Interpreter", "latex");
...
xticks([0 : 0.1 : 1.2]);
ylabel("$-w(x, t)$", "Fontsize", fontsize, "Interpreter", "latex");
ax = gca;
ax.YAxis.Exponent = 0;
ytickformat(gca, '%.5f');
legend([ws_dns_line, ws_fd_line, ws_nm_line], ["DNS", "Analytical: Finite difference", "Analytical: Normal modes"], ...
...
%% Loop over time
for k = 101 : 10 : length(T_VALS) - 1
    % Updates time
    t = T_VALS(k)
    
    %% DNS interface plotting
    nexttile(1, [2, 1]);
    title(sprintf("$t$ = %.3f", t * millisecond_scale), "Interpreter", "latex");
