...
for tIdx = 1 : length(ts)
    ...
    %% Plot pressures
    h(3 * (tIdx - 1) + 1) = plot(rs, ps_outer, 'Linewidth', 3, 'Color', lineColor, ...
        'Linestyle', '--', 'Displayname', 'Outer');
    h(3 * (tIdx - 1) + 2) = plot(rs, ps_inner, 'Linewidth', 3, 'Color', lineColor, ...
        'Linestyle', ':', 'Displayname', 'Inner');
    h(3 * (tIdx - 1) + 3) = plot(rs, ps_composite, 'Linewidth', 1.85, 'Color', lineColor, ...
        'Displayname', 'Composite');
    
    %% Plot turnover line
%     if tIdx == 2
%         h(7) = xline(epsilon * TimeDependents.ds, 'Linewidth', 0.5, 'Color', 0.5 * [1 1 1], ...
%             'Linestyle', '--');
%     end
%     
...
%% Add annotations
annotation('textbox',...
    [0.1325 0.5815 0.094 0.0675],...
    'String',{'$t = 0.0625$'}, 'interpreter', 'latex', 'Fontsize', 18, ...
    'Edgecolor', 'None');

annotation('textbox',...
    [0.57 0.5815 0.094 0.0675],...
...
exportgraphics(gca,'png/CompositePressureAxi.png', 'Resolution', 300);
exportgraphics(gca,'eps/CompositePressureAxi.eps', 'Resolution', 300);
